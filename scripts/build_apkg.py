...
# ─── s4-2a Prismen und Zylinder (Schwerpunkt) ───
s42a_cards = [
    ('Volumenformel des geraden Prismas?',
     '<b>V = G·h</b> — Grundfläche mal Höhe; gilt für jeden konstanten Querschnitt.'),
    ('Mantel und Oberfläche des Prismas?',
     '<b>M = U·h</b> (Umfang mal Höhe), <b>O = 2G + M</b>.'),
    ('Was besagt der <i>Satz von Cavalieri</i>?',
     'Körper mit <b>gleichen Querschnittsflächen in jeder Höhe</b> haben gleiches Volumen — darum gilt V = G·h auch für schiefe Prismen/Zylinder (mit der senkrechten Höhe).'),
    ('Quader: V, O und Raumdiagonale?',
     '<b>V = abc; O = 2(ab+ac+bc); d = √(a²+b²+c²)</b>.'),
    ('Woher kommt die Raumdiagonalen-Formel?',
     '<b>Zweimal Pythagoras</b>: erst Flächendiagonale √(a²+b²), dann senkrecht dazu mit c.'),
    ('Würfel: V, O, Diagonalen?',
     '<b>V = a³; O = 6a²</b>; Flächendiagonale a√2, Raumdiagonale <b>a√3</b>.'),
    ('Raumdiagonale eines Würfels mit a = 5?',
     '5√3 ≈ <b>8.66</b>.'),
    ('Zylinder: V, M, O?',
     '<b>V = πr²h; M = 2πrh; O = 2πr(r+h)</b>.'),
    ('Wie sieht das Netz des Zylinders aus?',
     'Zwei Kreise + <b>Mantel-Rechteck</b> mit Breite <b>2πr</b> (abgerollter Umfang) und Höhe h.'),
    ('Zylinder r=3, h=8: V und O?',
     'V = 72π ≈ <b>226.2</b>; O = 66π ≈ <b>207.3</b>.'),
    ('Radius aus V und h (Zylinder)?',
     '<b>r = √(V/(πh))</b> — erst teilen, dann Wurzel.'),
    ('Die Durchmesser-Falle?',
     'In allen Formeln steht der <b>Radius r = d/2</b>. Wer d einsetzt, bekommt das <b>Vierfache</b> des Volumens.'),
    ('Wie erscheinen Tiefenkanten im Schrägbild?',
     '<b>Halbiert und unter 45°</b>; die Frontfläche bleibt in wahrer Grösse.'),
    ('Was ist das Netz (die Abwicklung)?',
     'Alle Begrenzungsflächen <b>aufgeklappt in der Ebene</b> — Grundlage jeder Oberflächenberechnung.'),
    ('Umrechnung Liter ↔ cm³ / m³?',
     '<b>1 l = 1000 cm³; 1 m³ = 1000 l</b>.'),
    ('Verdoppelter Radius beim Zylinder: V und M?',
     'V <b>×4</b> (∼r²), M nur <b>×2</b> (∼r).'),
    ('Zusammengesetzte Körper: Volumen-Regel?',
     'Teilvolumina <b>addieren</b>, Bohrungen/Aussparungen <b>subtrahieren</b>.'),
    ('Zusammengesetzte Körper: Oberflächen-Regel?',
     'Nur <b>sichtbare</b> Flächen zählen — <b>Berührungsflächen</b> an Nahtstellen fallen weg.'),
    ('Prisma mit gleichseitigem Dreieck (a=6) als Grundfläche?',
     'G = (√3/4)·36 ≈ <b>15.59</b>; bei h=10: V ≈ 155.9, M = 18·10 = 180.'),
    ('Können Quaderkanten windschief sein?',
     '<b>Ja</b> — z.B. eine Bodenkante und eine dazu «versetzte» senkrechte Kante: weder parallel noch schneidend.'),
]

# ─── s4-2b Pyramiden, Kegel und Stümpfe (Schwerpunkt) ───
s42b_cards = [
    ('Volumenformel aller spitzen Körper?',
     '<b>V = ⅓·G·h</b> — ein Drittel des Prismas/Zylinders gleicher Grundfläche und Höhe (Füll-Experiment: 3 Füllungen).'),
    ('Die drei Strecken der quadratischen Pyramide?',
     'Körperhöhe h &lt; <b>Seitenhöhe h_s</b> (zur Kantenmitte) &lt; <b>Seitenkante s</b> (zur Ecke).'),
    ('Formel für die Seitenhöhe h_s?',
     '<b>h_s = √(h² + (a/2)²)</b> — Pythagoras mit der halben Kante.'),
    ('Formel für die Seitenkante s?',
     '<b>s = √(h² + (a√2/2)²)</b> — mit der halben Diagonale.'),
    ('Mantel und Oberfläche der quadratischen Pyramide?',
     '<b>M = 2·a·h_s</b> (vier Dreiecke), <b>O = a² + M</b>. Für den Mantel h_s, nie h!'),
    ('Pyramide a=6, h=4: h_s, V, O?',
     'h_s = √(16+9) = <b>5</b>; V = <b>48</b>; O = 36+60 = <b>96</b>.'),
    ('Kegel: V und Mantellinie?',
     '<b>V = ⅓πr²h</b>; <b>m = √(r²+h²)</b> — Hypotenuse im Achsenquerschnitt.'),
    ('Kegel: Mantel und Oberfläche?',
     '<b>M = πrm; O = πr(r+m)</b>.'),
    ('Öffnungswinkel des Kegels?',
     '<b>sin(w/2) = r/m</b> — der Winkel an der Spitze; w = 90° genau bei r = h.'),
    ('Der abgerollte Kegelmantel?',
     'Ein <b>Kreissektor</b>: Radius m, Winkel <b>α = (r/m)·360°</b> — der Sektorbogen ist der Grundkreisumfang.'),
    ('Vom Sektor zurück zum Kegel?',
     '<b>r = m·α/360°</b>, dann <b>h = √(m² − r²)</b>.'),
    ('Kegel r=3, h=4: m, V, M, w?',
     'm = <b>5</b>; V = 12π ≈ 37.7; M = 15π ≈ 47.1; w ≈ <b>73.7°</b>; Sektor 216°.'),
    ('Volumen des Pyramidenstumpfs?',
     '<b>V = (h/3)·(G₁ + √(G₁G₂) + G₂)</b>.'),
    ('Volumen des Kegelstumpfs?',
     '<b>V = (πh/3)·(R² + Rr + r²)</b>.'),
    ('Mantel des Kegelstumpfs?',
     '<b>M = π(R+r)·m</b> mit <b>m = √(h² + (R−r)²)</b>.'),
    ('Ähnlichkeit im Raum: die drei Skalierungen?',
     'Längen ∼ <b>k</b>, Flächen ∼ <b>k²</b>, Volumen ∼ <b>k³</b>.'),
    ('Kegel in halber Höhe geköpft: Volumenanteile?',
     'Restkegel <b>1/8</b> (k = ½ → k³), Stumpf <b>7/8</b> — nicht halbe-halbe!'),
    ('Stumpf ohne Stumpfformel?',
     '<b>Grosser spitzer Körper minus kleiner</b> — der kleine ist eine k-Kopie (Ähnlichkeit).'),
    ('Pyramidenstumpf G₁=64, G₂=16, h=6: V?',
     'V = 2·(64+32+16) = <b>224</b>.'),
    ('Schüttkegel: Höhe aus Böschungswinkel?',
     '<b>h = r·tan(β)</b> — z.B. r=4, β=30°: h ≈ 2.31.'),
]

# ─── s4-2c Kugel und zusammengesetzte Körper (Schwerpunkt) ───
s42c_cards = [
    ('Kugel: Volumen und Oberfläche?',
     '<b>V = (4/3)πr³; O = 4πr²</b> — die Oberfläche ist viermal der Grosskreis.'),
    ('Radius aus dem Volumen?',
     '<b>r = ∛(3V/(4π))</b> — Kubikwurzel, da V dreidimensional.'),
    ('Radius aus der Oberfläche?',
     '<b>r = √(O/(4π))</b> — Quadratwurzel, da O zweidimensional.'),
    ('Das Archimedes-Verhältnis?',
     'Kugel im engsten Zylinder (h = 2r): <b>V_Kugel : V_Zylinder = 2 : 3</b> — für jede Grösse.'),
    ('Kugel r=6: V und O?',
     'V ≈ <b>904.8</b>; O ≈ <b>452.4</b>.'),
    ('1-Liter-Kugel: Radius?',
     'r = ∛(3000/(4π)) ≈ <b>6.20 cm</b>.'),
    ('Formel der Kugelkappe (Fläche)?',
     '<b>A = 2πrh</b> — hängt nur von r und der Schichthöhe h ab.'),
    ('Formel des Kugelsegments (Körper)?',
     '<b>V = (πh²/3)·(3r − h)</b> — der eben abgeschnittene «Deckel».'),
    ('Formel des Kugelsektors?',
     '<b>V = (2πr²h)/3</b> — «Glace-Kegel»: Segment plus Kegel bis zum Zentrum.'),
    ('Grenzfall-Test der Segmentformel?',
     'h = r → <b>⅔πr³ (Halbkugel)</b> ✓; h = 2r → <b>(4/3)πr³ (ganze Kugel)</b> ✓.'),
    ('Halbkugel: V und vollständige Oberfläche?',
     '<b>V = ⅔πr³</b>; frei stehend <b>O = 3πr²</b> (gewölbt 2πr² + Schnittkreis πr²).'),
    ('Verdoppelter Kugelradius: V und O?',
     'V <b>×8</b> (k³), O <b>×4</b> (k²).'),
    ('Warum kühlen grosse Körper langsamer aus?',
     'Wärmeinhalt ∼ V ∼ k³, Abgabe ∼ O ∼ k² — das Verhältnis <b>O/V sinkt</b> mit der Grösse.'),
    ('Silo = Zylinder (r=3, h=8) + Halbkugel: V?',
     '72π + 18π = <b>90π ≈ 282.7 m³</b>.'),
    ('Silo-Aussenfläche ohne Boden?',
     'Zylindermantel + Halbkugel: 48π + 18π = <b>66π ≈ 207.3 m²</b> — Nahtstellen zählen nicht.'),
    ('Eiskugel (r) schmilzt in Kegel-Waffel (gleicher r): Mindesthöhe?',
     '<b>h = 4r</b> — aus (4/3)πr³ = ⅓πr²h; r² kürzt sich.'),
    ('Masse aus Volumen?',
     '<b>m = ρ·V</b> — z.B. Stahlkugel r=1 cm: V ≈ 4.19 cm³, m ≈ 32.9 g (ρ = 7.85).'),
    ('Grosse Bleikugel (r=6) → kleine Kugeln (r=2): wie viele?',
     'k = 3 → k³ = <b>27 Kugeln</b>.'),
    ('Grösste Kugel im Würfel: Radius und Füllanteil?',
     'r = a/2; Anteil <b>π/6 ≈ 52.36 %</b> — unabhängig von a (Ähnlichkeit).'),
    ('Zusammengesetzte Körper: die zwei Bilanz-Regeln?',
...
# ─── s1-2 Potenzen (Schwerpunkt) ───
s12_cards = [
    ('Definition der Potenz a^n?',
     '<b>a·a·…·a mit n Faktoren</b> — a heisst Basis, n Exponent. Abgekürzte Multiplikation.'),
    ('Was ist a⁰ — und warum?',
     '<b>a⁰ = 1</b> (a ≠ 0). Divisions-Treppe: 2³=8 → 2²=4 → 2¹=2 → 2⁰=1 — jeder Schritt teilt durch die Basis.'),
    ('Was bedeutet a⁻ⁿ?',
     '<b>a⁻ⁿ = 1/aⁿ</b> — der Kehrwert; z.B. 10⁻³ = 0.001, 2⁻⁴ = 1/16.'),
    ('Potenzgesetz: a^m · a^n?',
     '<b>a^(m+n)</b> — gleiche Basis: Exponenten addieren.'),
    ('Potenzgesetz: a^m : a^n?',
     '<b>a^(m−n)</b> — gleiche Basis: Exponenten subtrahieren.'),
    ('Potenzgesetz: (a^m)^n?',
     '<b>a^(m·n)</b> — Potenz der Potenz: Exponenten multiplizieren.'),
    ('Potenzgesetze bei gleichem Exponenten?',
     '<b>aⁿ·bⁿ = (ab)ⁿ</b> und <b>aⁿ:bⁿ = (a:b)ⁿ</b> — Basen verrechnen.'),
    ('Gibt es ein Gesetz für a² + a³?',
     '<b>Nein!</b> Für Summen gibt es kein Potenzgesetz. Probe a=2: 4+8=12 ≠ 32. Höchstens ausklammern: a²(1+a).'),
    ('a^(1/n) = ?',
     '<b>ⁿ√a</b> — der Bruchexponent 1/n ist die n-te Wurzel (a &gt; 0).'),
    ('a^(m/n) = ?',
     '<b>ⁿ√(a^m) = (ⁿ√a)^m</b> — Strategie: erst Wurzel (Zahl klein machen), dann Potenz.'),
    ('8^(2/3) = ?',
     '(³√8)² = 2² = <b>4</b>.'),
    ('16^(−1/2) = ?',
     '1/√16 = <b>0.25</b>.'),
    ('Hierarchie der Operationen?',
     '<b>Klammern → Potenzen → Punkt → Strich</b>. 2·3² = 18, nicht 36.'),
    ('−3² vs. (−3)²?',
     '<b>−3² = −9</b> (Potenz vor Vorzeichen), <b>(−3)² = +9</b> (Klammer macht −3 zur Basis).'),
    ('Wie wertet man Potenztürme aus?',
     '<b>Von oben:</b> 2^(3²) = 2⁹ = 512 — nicht (2³)² = 64. Konvention, kein Potenzgesetz!'),
    ('Exponentenschreibweise?',
     '<b>m · 10^k</b> mit Mantisse 1 ≤ m &lt; 10; z.B. 5 420 000 = 5.42·10⁶.'),
    ('Faustregel 2¹⁰ ≈ ?',
     '<b>2¹⁰ = 1024 ≈ 10³</b> (+2.4 %) — daher 2²⁰ ≈ 10⁶, 2³⁰ ≈ 10⁹ (Kibi/Mebi/Gibi).'),
    ('Papierfalten: Dicke nach n Faltungen?',
     '<b>d = 0.1 mm · 2ⁿ</b>; nach 42 Faltungen ≈ 440 000 km — am Mond vorbei (exponentielles Wachstum).'),
    ('Zinseszins-Formel?',
     '<b>K = K₀ · q ⁿ</b> mit q = 1 + p/100 — z.B. 5000·1.02¹⁰ ≈ 6094.97.'),
    ('√x · ⁴√x als eine Potenz?',
     'x^(1/2)·x^(1/4) = <b>x^(3/4) = ⁴√(x³)</b> — Gesetz 1 mit Bruchrechnung.'),
]

# ─── s1-3 Logarithmen (Schwerpunkt) ───
s13_cards = [
    ('Definition des Logarithmus?',
     '<b>a^x = b ⟺ x = log_a(b)</b> mit a, b &gt; 0, a ≠ 1 — die Umkehrfrage nach dem Exponenten: «a hoch was gibt b?»'),
    ('Warum muss b positiv sein?',
     'Potenzen positiver Basen sind <b>immer positiv</b> — «a hoch was gibt −5?» hat keine Antwort.'),
    ('Die vier Sofortwerte?',
     '<b>log_a(1) = 0; log_a(a) = 1; log_a(a^k) = k; a^(log_a b) = b</b>.'),
    ('Die drei Spezialbasen?',
     '<b>lg = log₁₀</b> (dekadisch), <b>ln = log_e</b> (natürlich), <b>log₂</b> (binär).'),
    ('log₂(32) = ?',
     '<b>5</b> — denn 2⁵ = 32.'),
    ('lg(0.001) = ?',
     '<b>−3</b> — Argumente zwischen 0 und 1 geben negative Logarithmen.'),
    ('log₅(1/25) = ?',
     '1/25 = 5⁻² → <b>−2</b>.'),
    ('Logarithmengesetz: Produkt?',
     '<b>log_a(u·v) = log_a(u) + log_a(v)</b> — Produkt wird Summe.'),
    ('Logarithmengesetz: Quotient?',
     '<b>log_a(u/v) = log_a(u) − log_a(v)</b> — Quotient wird Differenz.'),
    ('Logarithmengesetz: Potenz?',
     '<b>log_a(u^k) = k·log_a(u)</b> — der Exponent fällt als Faktor herunter.'),
    ('Gibt es ein Gesetz für log(u+v)?',
     '<b>Nein!</b> Probe: lg(100) = 2, aber lg 10 + lg 90 ≈ 2.95 — kein Gesetz für Summen.'),
    ('lg(5) + lg(2) = ?',
     'lg(5·2) = lg(10) = <b>1</b>.'),
    ('Der Basiswechselsatz?',
     '<b>log_a(b) = lg(b)/lg(a) = ln(b)/ln(a)</b> — die Basis steht im Nenner.'),
    ('log₄(8) exakt?',
     '(3·lg2)/(2·lg2) = <b>3/2</b> — Kontrolle: 4^1.5 = 8 ✓'),
    ('Wie löst man a^x = b?',
     '<b>Beide Seiten logarithmieren</b>, Gesetz 3 holt x herunter: x = lg(b)/lg(a) = log_a(b).'),
    ('3^x = 20: Lösung und Fenster?',
     'Fenster: 9 &lt; 20 &lt; 27 → 2 &lt; x &lt; 3; <b>x = lg20/lg3 ≈ 2.73</b>.'),
    ('Verdopplungszeit bei 3 % Wachstum?',
     '1.03ⁿ = 2 → <b>n = lg2/lg1.03 ≈ 23.45 Jahre</b> (70er-Regel: 70/3 ≈ 23).'),
    ('pH-Wert-Definition?',
     '<b>pH = −lg[H⁺]</b>; z.B. [H⁺] = 3.2·10⁻⁵ → pH ≈ 4.49. Eine Stufe = Faktor 10.'),
    ('Stellenzahl einer Zahl N?',
     '<b>⌊lg N⌋ + 1</b> — so hat 2¹⁰⁰ genau ⌊30.103⌋+1 = <b>31 Stellen</b>.'),
    ('log_a(b) · log_b(a) = ?',
...
SP_DECKS = [
    ...
    ('s4-2a-prismen-zylinder', '4.2a Prismen und Zylinder',
     'V = G·h, Cavalieri, Quader-Diagonalen, Zylinder-Netz, Formeln umstellen, zusammengesetzte Körper.', s42a_cards),
    ('s4-2b-pyramiden-kegel-stuempfe', '4.2b Pyramiden, Kegel und Stümpfe',
