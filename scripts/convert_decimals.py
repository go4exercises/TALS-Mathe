r"""
Convert Dezimalkomma → Dezimalpunkt in alle HTML-Dateien des Lehrmittels.

Konvention (gemäss STYLEGUIDE §2.4): Dezimal**punkt**, nicht Dezimalkomma.
...
def process_text(text, allow_coord_replace=False):
    ...
    Ganzzahlen zu `(a | b)` umgestellt (STYLEGUIDE §2.4, Variante B). Sonst
    bleiben sie unangetastet — wichtig für Dateien, in denen `(a, b)` als
    Aufzählung von zwei Datenwerten gemeint ist (z.B. Klassenhäufigkeiten).
    ...
    # Inline-Event-Handler (onclick, onchange, onload, etc.) — enthalten JS-Code
    # mit Funktions-Argumenten wie `f(1, 2, 3)`, die KEINE Dezimalzahlen sind.
    ...
    # LaTeX-Subscripts mit Index-Liste schützen: x_{1,2}, x_{1, 2}, a_{i,j} etc.
    # (Komma trennt hier Indizes, ist kein Dezimalkomma)
    text = re.sub(r'_\{[0-9]+,\s*-?[0-9]+\}', stash, text)
    text = re.sub(r'_\{[0-9]+(?:,\s*-?[0-9]+){2,}\}', stash, text)

    # Punkt-Koordinaten (a, b), (a, b, c), Vektor-Tupel etc. schützen — Komma
    # ist hier Trennzeichen, kein Dezimalkomma. Umfasst 2-Tupel `(0, 0)`,
    # 3-Tupel `(3, 4, 5)`, 4-Tupel `(180, 120, 30, 0.55)` etc.
    # Letzteres Beispiel zeigt: solange mindestens eine reine Ganzzahl-Komponente
    # darin ist, ist es typisch ein Tupel, kein Dezimalkomma — wir schützen
    # 2-Tupel sicherheitshalber alle aus reinen Ganzzahlen.
    text = re.sub(
        r'\(\s*-?[0-9]+(?:\s*,\s*-?[0-9]+){1,3}\s*\)',
        stash, text)

    # 2-Tupel-Koordinaten (a, b) → (a | b) gemäss STYLEGUIDE §2.4 (Variante B).
    # Nur wenn der Aufrufer das explizit freigibt (allow_coord_replace=True),
    # weil in manchen Dateien `(a, b)` eine Aufzählung von Datenwerten ist
    # (z.B. Klassenhäufigkeiten in Statistik-Aufgaben).
    n_coords = [0]
...
def process_file(filepath, dry_run=False):
    # Whitelist: Dateien, in denen 2-Tupel `(a, b)` echte Punkt-Koordinaten sind
    # und gemäss STYLEGUIDE §2.4 auf `(a | b)` umzustellen sind. In allen anderen
    # Dateien können `(a, b)`-Klammern Datenwert-Aufzählungen sein (z.B.
    # Klassenhäufigkeiten) und dürfen nicht angetastet werden.
    COORD_WHITELIST = {
...
def verify_no_residuals(filepath):
    """Prüft, ob nach Konvertierung noch problematische Stellen übrig sind.

    Bewusst stehen bleiben dürfen (= keine Residuen):
    - LaTeX-Subscripts `x_{1,2}`, `a_{1,2,3}` etc. (Index-Listen)
    - Aufzählungs-Tupel `(3, 4, 5)`, `(19, 34, 27)` (pyth. Tripel, Vektoren)
    - Google-Fonts-URL-Parameter (`@0,400;0,700` etc.)
    - CSS, SVG, SVG-Attribute
    """
    with open(filepath, 'r', encoding='utf-8') as f:
        text = f.read()
    ...
    masked = re.sub(r"\son\w+='[^']*'", '<S>', masked)
    # Erlaubte Komma-Stellen explizit ausmaskieren
    ...
    # Punkt-Paare und Tupel: (a,b), (a, b, c), (0,-30), (1, 2, 3) etc.
    masked = re.sub(r'\(\s*-?[0-9]+(?:\s*,\s*-?[0-9]+){1,3}\s*\)', '<S>', masked)
    # 2-Tupel sind nicht "Residuum" — sind absichtlich erhalten geblieben in
    # Dateien ausserhalb der Whitelist (z.B. Datenwert-Aufzählungen).
    masked = re.sub(r'\(\s*-?[0-9]+\s*,\s*-?[0-9]+\s*\)', '<S>', masked)

    # In <script>: nur restliches {,} ist problematisch
    script_braces = 0
