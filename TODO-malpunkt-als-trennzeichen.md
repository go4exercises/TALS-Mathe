...
**Stand: 26. August 2026.** Vollständige Suche über alle `*.html` und `*.js` im Repo.
...
- [ ] **Zeile 1798** — Canvas-Beschriftung im Äquivalenz-Widget — trennt zwei Wertaussagen.
  ```
...
- [ ] **Zeile 858** — Tabellenzelle «Drei Fälle» — drei Fallaussagen mit `·` verkettet.
  ```
...
- [ ] **Zeile 1339** — Live-Anzeige LGS — `2·x + y = 4 &nbsp;·&nbsp; …`: das Trennzeichen steht direkt neben einem echten Malpunkt.
  ```
  eqEl.innerHTML = `2·x + y = 4 &nbsp;·&nbsp; ${g2links} = ${c} &nbsp; → &nbsp; D = 2 − ${m} = ${D.toFixed(2)} ≠ 0 → ` +
  ```
- [ ] **Zeile 1344** — Wie Zeile 1339 (Zweig «identisch»).
  ```
  eqEl.innerHTML = `2·x + y = 4 &nbsp;·&nbsp; 2·x + y = 4 &nbsp; → &nbsp; D = 0 und beide Gleichungen sind gleich: ` +
  ```
- [ ] **Zeile 1349** — Wie Zeile 1339 (Zweig «parallel»).
  ```
...
- [ ] **Zeile 1209** — Tabellenzelle «Argument · Wert» → Inhalt `\(x\) · \(f(x) = y\)` liest sich als Produkt.
  ```
  <td>\(x\) · \(f(x) = y\)</td>
  ```
...
- [ ] **Zeile 939** — Preset-Beschreibung: `f(x) = x · Identität: m = 1, b = 0 · 45°-Gerade …`.
  ```
  {label:'Identität', desc:'f(x) = x · Identität: m = 1, b = 0 · 45°-Gerade durch Ursprung', m:1, b:0, vert:false, color…
  ```
- [ ] **Zeile 940** — Preset-Beschreibung: `f(x) = 2 · Konstante Funktion …` — hier besonders heikel, weil `2 ·` wie ein Faktor aussieht.
  ```
  {label:'Konstant', desc:'f(x) = 2 · Konstante Funktion: m = 0 · waagrechte Gerade', m:0, b:2, vert:false, color:'#b85c…
  ```
- [ ] **Zeile 944** — Preset-Beschreibung (senkrechte Geraden) — Mischung aus echtem Malpunkt und Trennzeichen in derselben Zeile.
  ```
  {label:'Senkrecht ⊥', desc:'Zwei senkrechte Geraden: m₁ · m₂ = 2 · (−0.5) = −1', m:null, b:null, vert:false, color:'#1…
  ```
- [ ] **Zeile 1172** — Live-Anzeige Steigungsdreieck: `P(…), Q(…) &nbsp;·&nbsp; Δx = 0 → …`.
  ```
  eqEl.innerHTML = `P(${nz(x1)} | ${nz(y1)}), Q(${nz(x2)} | ${nz(y2)}) &nbsp;·&nbsp; Δx = 0 → ` +
  ```
- [ ] **Zeile 1181** — Live-Anzeige Steigungsdreieck, zweiter Zweig.
  ```
...
- [ ] **Zeile 945** — Canvas-Beschriftung: `MW = … · Spannweite = …`.
  ```
  beschriftung(ctx, 'MW = ' + meanA.toFixed(2) + ' · Spannweite = ' + spanA.toFixed(1),
  ```
- [ ] **Zeile 948** — Wie Zeile 945 (zweiter Datensatz).
  ```
...
- [ ] **Zeile 775** — SVG-Text: `Scheitel: α = α · Neben: α + β = 180°`.
  ```
...
- [ ] **Zeile 379** — Live-Anzeige `#gl-val` (HTML-Startwert): `φ₁ = 30° · φ₂ = 150°`.
  ```
  <div class="ll-val" id="gl-val">φ₁ = 30° · φ₂ = 150°</div>
  ```
- [ ] **Zeile 544** — Live-Anzeige `#kk-val` (HTML-Startwert).
  ```
  <div class="ll-val" id="kk-val">φ₁ = 30° · φ₂ = 150° (= 180° − φ₁)</div>
  ```
- [ ] **Zeile 556** — Live-Anzeige `#lt-frage`: `Gleichung: sin φ = 0.5 · Rechner: φ₁ = 30°` — `0.5 · Rechner` liest sich als Produkt.
  ```
  <div class="lt-frage" id="lt-frage">Gleichung: sin φ = 0.5 · Rechner: φ₁ = 30°</div>
  ```
- [ ] **Zeile 963** — JS-Partner zu Zeile 379.
  ```
  valOut.textContent = `φ₁ = ${fmt(arr[0])} · φ₂ = ${fmt(arr[1])}`;
  ```
- [ ] **Zeile 994** — JS-Partner zu Zeile 544.
  ```
  valOut.textContent = `φ₁ = ${fmt(arr[0])} · φ₂ = ${fmt(arr[1])}`;
  ```
- [ ] **Zeile 1013** — Dritte Stelle mit `φ₁ = … · φ₂ = …`.
  ```
  valOut.textContent = `φ₁ = ${fmt(arr[0])} · φ₂ = ${fmt(arr[1])}`;
  ```
- [ ] **Zeile 1535** — Live-Anzeige Spiegelachse: `φ₁ = … · φ₂ = … — spiegelbildlich …`.
  ```
  val.textContent = `φ₁ = ${sol[0].toFixed(1)}° · φ₂ = ${sol[1].toFixed(1)}° — spiegelbildlich zur ${achse} Achse`;
  ```
- [ ] **Zeile 1576** — JS-Partner zu Zeile 556.
  ```
...
- [ ] **Zeile 774** — Live-Anzeige: `(a+b)² = … · a²+b² = … · Differenz = …` — drei Aussagen in einer Zeile.
  ```
...
- [ ] **Zeile 377** — Live-Anzeige `#sc-eq` (HTML-Startwert): `… ⇒ x = 3 · Probe: √25 = 5 ✓ · L = {3}`.
  ```
  <div class="fl-eq" id="sc-eq">6x + 7 = 25 ⇒ x = 3 · Probe: √25 = 5 ✓ · L = {3}</div>
  ```
- [ ] **Zeile 416** — Live-Anzeige `#br-eq` (HTML-Startwert): `D = ℝ \ {3} · Kandidat x = 2 ∈ D ⇒ L = {2}`.
  ```
  <div class="fl-eq" id="br-eq">D = ℝ \ {3} · Kandidat x = 2 ∈ D ⇒ L = {2}</div>
  ```
- [ ] **Zeile 908** — JS-Partner zu Zeile 377.
  ```
  {c: 5, eq: '6x + 7 = 25 ⇒ x = 3 · Probe: √25 = 5 ✓ · L = {3}'},
  ```
- [ ] **Zeile 909** — JS-Partner zu Zeile 377 (Scheinlösungs-Zweig).
  ```
  {c: -5, eq: '6x + 7 = 25 ⇒ x = 3 · Probe: √25 = 5 ≠ −5 ✗ · L = { } (Scheinlösung!)'},
  ```
- [ ] **Zeile 1017** — JS-Partner zu Zeile 416 (Pol-Zweig).
  ```
  ? `D = ℝ \\ {2} · Kandidat x = 2 ∉ D (Pol!) ⇒ <span class="tx-orange">L = { }</span>`
  ```
- [ ] **Zeile 1018** — JS-Partner zu Zeile 416.
  ```
...
- [ ] **Zeile 389** — Live-Anzeige `#gl-eq` (HTML-Startwert): `x = log₃(8) = … ≈ 1.89 · L = {1.89}`.
  ```
  <div class="fl-eq" id="gl-eq">x = log₃(8) = ln 8 / ln 3 ≈ 1.89 · L = {1.89}</div>
  ```
- [ ] **Zeile 495** — Live-Anzeige `#ld-eq` (HTML-Startwert): vier Aussagen mit `·` verkettet.
  ```
  <div class="fl-eq" id="ld-eq">D: x &gt; 3 · x₁ = 5 ∈ D ✓ · x₂ = −2 ∉ D (Scheinlösung) · L = {5}</div>
  ```
- [ ] **Zeile 906** — JS-Partner zu Zeile 389.
  ```
  {lhs: x => Math.pow(3, x), rhs: x => 8, eq: 'x = log₃(8) = ln 8 / ln 3 ≈ 1.89 · L = {1.89}',
  ```
- [ ] **Zeile 1011** — JS-Partner zu Zeile 495.
  ```
...
- [ ] **Zeile 498** — Bildlegende: `Parabel \(f(x)=x^2+c\) violett · Gerade \(g(x)=x+1\) blau · …` (niedrige Prio: Prosa, aber Math direkt links des Punktes).
  ```
...
- [ ] **Zeile 492** — Live-Anzeige `#pa-eq` (HTML-Startwert): `gerade · f(−x) = +f(x) · achsensymmetrisch …`; JS dazu ab Zeile 1300.
  ```
  <div class="fl-eq" id="pa-eq">gerade · f(−x) = +f(x) · achsensymmetrisch zur y-Achse</div>
  ```
- [ ] **Zeile 565** — Live-Anzeige `#hy-asym` (HTML-Startwert): `Polgerade: x = 1 · Asymptote: y = −1`.
  ```
  <div class="fl-eq" id="hy-asym" style="font-size:0.95rem;margin-top:4px">Polgerade: x = <span class="tx-blau">1</span>…
  ```
- [ ] **Zeile 1149** — JS-Partner zu Zeile 565.
  ```
...
- [ ] **Zeile 499** — Live-Anzeige `#tf-eq` (HTML-Startwert): `y = √x · D = [0; ∞[`.
  ```
  <div class="fl-eq" id="tf-eq">y = √x · D = [0; ∞[</div>
  ```
- [ ] **Zeile 1219** — JS-Partner zu Zeile 499.
  ```
...
- [ ] **Zeile 356** — Live-Anzeige `#ek-eq`: `sin(7π/12) ≈ 0.97 · x ≈ 105°`.
  ```
  <div class="fl-eq" id="ek-eq">sin(7π/12) ≈ 0.97 · x ≈ 105°</div>
  ```
- [ ] **Zeile 494** — Live-Anzeige `#tr-info`: `Amplitude 1 · Periode p = 2π · Mittellinie y = 0`.
  ```
  <div class="fl-eq" id="tr-info" style="font-size:0.95rem;margin-top:4px">Amplitude 1 · Periode p = 2π · Mittellinie y …
  ```
- [ ] **Zeile 1188** — JS-Partner zu Zeile 494.
  ```
...
- [ ] **Zeile 562** — Live-Anzeige `#wa-eq` (HTML-Startwert): `Boden [−2; 3] · Minimum |a−b| = 5`.
  ```
  <div class="fl-eq" id="wa-eq">Boden [−2; 3] · Minimum |a−b| = 5</div>
  ```
- [ ] **Zeile 1162** — JS-Partner zu Zeile 562 (Zweig «Punkt»).
  ```
  ? `Boden zu einem Punkt · Minimum = 0 · reines V bei x = ${a}`
  ```
- [ ] **Zeile 1163** — JS-Partner zu Zeile 562 (Zweig «Intervall»).
  ```
...
- [ ] **Zeile 859** — Live-Anzeige: `V = … cm³ · O = … cm² · d = … cm`.
  ```
  `V = ${fmt2(V)} cm³ · O = ${fmt2(O)} cm² · d = ${d.toFixed(2)} cm`;
  ```
- [ ] **Zeile 912** — Live-Anzeige: `V = … · M = … · O = …`.
  ```
...
- [ ] **Zeile 854** — Live-Anzeige: `m = … · V = … · M = … · w = …°` — vier Werte.
  ```
...
- [ ] **Zeile 287** — Live-Anzeige `#fl-eq` (HTML-Startwert): `… = 5.00 m/s · 36.9° zur Querrichtung` — Zahl · Zahl, der schlimmste Fall.
  ```
  <div class="fl-eq" id="fl-eq">|v| = √(3.0² + 4.0²) = 5.00 m/s · 36.9° zur Querrichtung</div>
  ```
- [ ] **Zeile 407** — Live-Anzeige `#ad-eq` (HTML-Startwert): `a + b = (4 | 3) · |a + b| = 5.00`.
  ```
  <div class="fl-eq" id="ad-eq">a + b = (4 | 3) · |a + b| = 5.00</div>
  ```
- [ ] **Zeile 864** — JS-Partner zu Zeile 287.
  ```
  document.getElementById('fl-eq').innerHTML =
  ```
- [ ] **Zeile 905** — JS-Partner zu Zeile 407.
  ```
  `a + b = (${String(s1).replace('-', '−')} | ${String(s2).replace('-', '−')}) · |a + b| = ${betrag.toFixed(2)}`;
  ```
- [ ] **Zeile 1018** — Live-Anzeige Richtungswinkel: `r = … &nbsp;·&nbsp; a₁ = 0: …`.
  ```
  eqEl.innerHTML = `r = ${nz(r)} &nbsp;·&nbsp; a₁ = 0: der Quotient a₂/a₁ ist <strong>nicht definiert</strong> &nbsp;→&n…
  ```
- [ ] **Zeile 1024** — Live-Anzeige Richtungswinkel: `r = … &nbsp;·&nbsp; arctan(…) = …°`.
  ```
...
- [ ] **Zeile 276** — Bildlegende: `F = 50 N · s = 8 m · Projektion der Kraft grün` — auf einer Seite, die `·` gleichzeitig als Skalarprodukt-Zeichen verwendet.
  ```
  <p>F = 50 N · s = 8 m · Projektion der Kraft grün</p>
  ```
- [ ] **Zeile 382** — Bildlegende: `\(\vec a = …\) fest (blau) · \(\vec b\) drehbar … · Projektion …` (gleiche Seite, gleiche Verwechslungsgefahr).
  ```
...
- [ ] **Zeile 86** — Fünf-Punkte-Zusammenfassung `min · \(Q_1\) · \(\tilde x\) · \(Q_3\) · max`.
  ```
...
- [ ] **Zeile 47** — `Sinus = … · Cosinus = … · Tangens = …`.
  ```
...
- [ ] **Zeile 187** — Lösungszeile.
  ```
  <p>(a) \( x = 4 \) · (b) \( 3^x = 3^{-3} \Rightarrow x = -3 \) ·
  ```
- [ ] **Zeile 216** — Lösungszeile.
  ```
...
- [ ] **Zeile 89** — `\(a < 0\): Maximum \(v\) · \(a > 0\): Minimum \(v\)`.
  ```
...
- [ ] **Zeile 188** — Lösungszeile.
  ```
  <p>(a) \( D = \mathbb{R}_0^+ \) · (b) \( D = \mathbb{R} \setminus \{0\} \) ·
  ```
- [ ] **Zeile 193** — Lösungszeile.
  ```
...
- [ ] **Zeile 62** — Zwei Formeln in einer Tabellenzelle mit `·` getrennt.
  ```
...
- [ ] **Zeile 65** — `… = 0 \) → \(t\) · (3) Abstand …` — Schrittzähler mit `·` getrennt.
  ```
...
- [ ] **Zeile 64** — `1 Lösung = Durchstosspunkt · 0 = parallel · ∞ = enthalten`.
  ```
...
- [ ] `grundlagen/g1-2-zahlen-grundoperationen.html` — Zeile 1576
- [ ] `grundlagen/g1-3-algebraische-terme.html` — Zeile 2909
- [ ] `grundlagen/g2-3-lineare-gleichungssysteme.html` — Zeile 1315, 1317
- [ ] `grundlagen/g4-0-praxisbeispiel-bm2-klasse.html` — Zeile 957
- [ ] `schwerpunkt/s1-2-potenzen.html` — Zeile 949
- [ ] `schwerpunkt/s2-1-grundlagen.html` — Zeile 800
- [ ] `schwerpunkt/s4-3a-vektorbegriff-komponenten.html` — Zeile 993
