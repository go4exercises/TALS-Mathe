#!/usr/bin/env python3
"""
Pre-Flight-Check für TALS-Mathe-Themenseiten (grundlagen/ und schwerpunkt/).

Zwei Stufen:
1. Schnelle, abhängigkeitsfreie Eigen-Checks (Tag-Bilanz, ß, Dezimalkomma in Math,
   doppelte IDs, Skelett, Phantom-Klassen, mathlib-Abhängigkeit, Ressourcen-Marker).
...
Aufruf:  python3 .claude/skills/preflight/preflight.py grundlagen/*.html schwerpunkt/*.html
...
# Klassennamen, die im Mathe-CSS NICHT existieren ('rlp' ist in Mathe legitim!).
PHANTOM_CLASSES = [
...
def main(argv):
    args = argv[1:]
    if not args:
        print("Aufruf: python3 .claude/skills/preflight/preflight.py grundlagen/*.html schwerpunkt/*.html")
        return 2
